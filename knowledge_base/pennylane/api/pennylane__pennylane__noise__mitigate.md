...
api_version: v0.45.1
doc_type: api
source_path: pennylane/noise/mitigate.py
source_url: https://github.com/PennyLaneAI/pennylane/blob/5f61ce25df3cc28a1ac785d20e47d70761202ed2/pennylane/noise/mitigate.py
license: Apache-2.0
---

## Module `pennylane/noise/mitigate.py`
...
```python
def exponential_extrapolate(x, y, asymptote=None, eps=1e-06)
```

Extrapolate to the zero-noise limit using an exponential model (:math:`Ae^{Bx} + C`). This
...
```python
def mitigate_with_zne(tape: QuantumScript, scale_factors: Sequence[float], folding: callable, extrapolate: callable, *, folding_kwargs: dict[str, Any] | None=None, extrapolate_kwargs: dict[str, Any] | None=None, reps_per_factor=1) -> tuple[QuantumScriptBatch, PostprocessingFn]
```
