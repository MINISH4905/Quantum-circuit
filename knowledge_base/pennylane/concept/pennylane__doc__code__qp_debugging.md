...
api_version: v0.45.1
doc_type: concept
source_path: doc/code/qp_debugging.rst
source_url: https://github.com/PennyLaneAI/pennylane/blob/5f61ce25df3cc28a1ac785d20e47d70761202ed2/doc/code/qp_debugging.rst
license: Apache-2.0
---
...
When this code reaches ``qp.breakpoint()`` it will pause and launch an interactive
...
.. code-block:: console
    ...
    [pldb] longlist
      3  	@qp.qnode(qp.device('default.qubit', wires=(0,1,2)))
      4  	def circuit(x):
      5  	    qp.breakpoint()
      6
      7  ->	    qp.Hadamard(wires=0)
      8  	    qp.CNOT(wires=(0,2))
      9
     10  	    for w in (0, 1, 2):
     11  	        qp.RX(2*x, wires=w)
     12
     13  	    qp.breakpoint()
     14  	    qp.RX(-x, wires=1)
     15  	    return qp.sample()
...
.. code-block:: console
    ...
    0.0
    [pldb] qp.debug_expval(qp.X(0) @ qp.X(2))
    0.9999999999999996
...
.. code-block:: console
    ...
    1: ────│───RX─┤
    2: ────╰X──RX─┤
    [pldb] qp.RZ(0.5*x, wires=0)
    RZ(0.61725, wires=[0])
    [pldb] qp.CZ(wires=(1,2))
    CZ(wires=[1, 2])
    [pldb] qtape = qp.debug_tape()
    [pldb] print(qtape.draw(wire_order=(0,1,2)))
    0: ──H─╭●──RX──RZ─┤
