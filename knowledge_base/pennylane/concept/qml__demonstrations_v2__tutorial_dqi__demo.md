...
doc_type: concept
source_path: demonstrations_v2/tutorial_dqi/demo.py
source_url: https://github.com/PennyLaneAI/qml/blob/cf4629f869bf4dbc8ce0f9a5a3256eda138c7bbb/demonstrations_v2/tutorial_dqi/demo.py
license: Apache-2.0
---
...
Later, we’ll use samples generated by the DQI algorithm and compare their quality, in terms of the objective function, to this 
...
# Define parameters of max-XORSAT problem Bx=(mod2)v with m=5 and n=4
...
To prepare it, we will use ``qp.StatePrep`` in our ``embed_weights`` function shown below.

```python
def embed_weights(w_k, weight_register):
    """Prepares the weight register in superposition of given coefficients."""
    qp.StatePrep(w_k, wires=weight_register, pad_with=0)
```
...
prepare Dicke states :math:`|D_q^m\rangle` for :math:`q<k`, when given the appropriate input.
...
a controlled way via ``qp.ctrl()`` in the quantum function ``weight_error_prep``, after preparing 
...
def SCS(m, k):
    ...
    # Two-qubit gate
    qp.CNOT(wires=[m - 1, m])
    qp.CRY(2 * pnp.arccos(pnp.sqrt(1 / m_angle)), wires=[m, m - 1])
    qp.CNOT(wires=[m - 1, m])

    # k-1 three-qubit gates
    for l in range(2, k + 1):
        qp.CNOT(wires=[m - l, m])
        qp.ctrl(qp.RY, (m, m - l + 1))(2 * pnp.arccos(pnp.sqrt(l / m_angle)), wires=m - l)
        qp.CNOT(wires=[m - l, m])
...
def weight_error_prep(m, n, l):
    ...
    qp.ctrl(prepare_dicke_state, weight_register, control_values=(0,1))(m, k=1)
    qp.ctrl(prepare_dicke_state, weight_register, control_values=(1,0))(m, k=2)
...
def phase_Z(v):
    ...
    for i in range(len(v)):
        qp.cond(v[i],qp.Z)(wires=i + num_weight_qubits)
...
def encode_v(m, n, l):
    ...
    qp.StatePrep(raw_state_vector, wires=range(0, m + num_weight_qubits))
...
def B_T_multiplication(B_T, n_register):
    ...
    for row_index, row in enumerate(B_T):
        for col_index, element in enumerate(row):
            if element == 1:
                qp.CNOT(wires=[m_register[col_index], n_register[row_index]])
...
@partial(qp.set_shots, shots=n_samples)
@qp.qnode(dev)
def decoding(m, n, l):
    """Quantum circuit decoding and uncomputing error register"""

    # Load the previous state
    qp.StatePrep(raw_state_vector, wires=range(0, m + n + num_weight_qubits))

    # Uncompute syndrome register using a Lookup table
    for syndrome, error in decoding_table:
        for i in range(len(error)):
            if error[i] == 1:
                qp.ctrl(qp.X, n_register, control_values=syndrome)(m_register[i])
...
def DQI(m, n, l):
    ...
    return qp.counts(wires=n_register)
```
...
all solutions with a given objective value.

Conclusion
----------

In this tutorial, we implemented the Decoded Quantum Interferometry (DQI) algorithm to solve the
...
.. [#Bartschi2019]
   ...
   `arXiv:1904.07358v1 <https://arxiv.org/abs/1904.07358v1>`__, 2019.
...
.. [#Patamawisut2025]
   ...
   `arXiv:2504.18334v2 <https://arxiv.org/abs/2504.18334v2>`__, 2025.

.. [#Classiq2025]

   Classiq. 
   “Decoded Quantum Interferometry Algorithm.”
   `Classiq library <https://docs.classiq.io/latest/explore/algorithms/search_and_optimization/dqi/dqi_max_xorsat/>`__, 2025.
